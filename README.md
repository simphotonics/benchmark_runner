...
To export benchmark scores use the `benchmark_runner` sub-command `export`:
```
$ dart run benchmark_runner export --outputDir=scores --extension=csv path
```
In the example above, `path` is scanned for `*_benchmark.dart`
files. For each benchmark file, a corresponding file `*_benchmark.csv` is
written to the directory `scores`.
The following points are worth considering:
* If `--outputDir` is omitted, it defaults to `benchmark`.
*  The output directory must *exist* and the user
must have *write access*.
* When exporting benchmark scores to a file
and the output generated by the score emitter is colorized,
it is recommended to use the `benchmark_runner` option `--isMonochrome`, to
...
an object of a type that implements the interface
`ScoreEmitter`. It can be used to customize the benchmark scores e.g.
...
of the async benchmark functions *and* the enclosing async group.
